...
TEST(QuadrotorVecEnv, StepEnv) {
  ...
  for (int i = 0; i < SIM_STEPS_N; i++) {
    ...
    vec_env.step(act, obs, reward, done, is_collision, extra_info);
  }
  ...
  EXPECT_FALSE(vec_env.step(act, obs, reward, done, is_collision, extra_info));
