...
class VisionEnv final : public EnvBase {
 public:
  ...
  VisionEnv();
  VisionEnv(const std::string &cfg_path, int env_id);
  VisionEnv(const YAML::Node &cfg_node, int env_id);
  ~VisionEnv() override;

  // - public OpenAI-gym-style functions
  bool reset(Ref<Vector<>> obs) override;
  bool reset(const Ref<Vector<>>& obs, bool random);
  ...
  bool getImage(Ref<ImgVector<>> img, bool rgb = true) override;
  ...
  bool addQuadrotorToUnity(std::shared_ptr<UnityBridge> bridge) override;
  ...
  bool simDynamicObstacles(Scalar dt);
  ...
  //
  int getNumDetectedObstacles() const;
