...
bool VisionEnv::reset(Ref<Vector<>> obs) {
  ...
  // reset position
  if (drone_position.empty()) {
      std::uniform_real_distribution<Scalar> uniform_dist_x{-5, 1.0};
      std::uniform_real_distribution<Scalar> uniform_dist_y{-5.0, 5.0};
      std::uniform_real_distribution<Scalar> uniform_dist_z{0.0, 10.0};

      quad_state_.x(QS::POSX) = uniform_dist_x(random_gen_);
      quad_state_.x(QS::POSY) = uniform_dist_y(random_gen_);
      quad_state_.x(QS::POSZ) = uniform_dist_z(random_gen_);

  } else{
      quad_state_.x(QS::POSX) = drone_position.at(0);
      quad_state_.x(QS::POSY) = drone_position.at(1);
      quad_state_.x(QS::POSZ) = drone_position.at(2);
  }
...
bool VisionEnv::reset(const Ref<Vector<>>& obs, bool random) { return reset(obs); }
...
bool VisionEnv::getObstacleState(Ref<Vector<>> obs_state) {
  if (dynamic_objects_.empty() && static_objects_.empty()) {
//    logger_.error("No dynamic or static obstacles.");
  ...
  for (auto & dynamic_object : dynamic_objects_) {
    ...
    Vector<3> delta_pos = dynamic_object->getPos() - quad_state_.p;
    ...
    Scalar obs_radius = dynamic_object->getScale()[0] / 2;
  ...
  for (auto & static_object : static_objects_) {
    // compute relative position vector
    Vector<3> delta_pos = static_object->getPos() - quad_state_.p;
    relative_pos.push_back(delta_pos);
    ...
    // store the obstacle radius
    Scalar obs_radius = static_object->getScale()[0] / 2;
    obstacle_radius_.push_back(obs_radius);
...
bool VisionEnv::isCollision() const {
...
bool VisionEnv::loadParam(const YAML::Node &cfg) {
  if (cfg["environment"]) {
      if (cfg["environment"]["static_obstacles"]){
          static_object_csv_ = cfg["environment"]["static_obstacles"].as<std::string>();
      }

      if (cfg["environment"]["dynamic_obstacles"]){
          dynamic_object_yaml = cfg["environment"]["dynamic_obstacles"].as<std::string>();
      }

      if (cfg["environment"]["static_gates"]){
          static_gates_csv = cfg["environment"]["static_gates"].as<std::string>();
      }
      else {
          static_gates_csv = std::string("/home/tommaso/icra22_competition_ws/src/agile_flight/flightmare/assets/default_empty.csv");
      }

      if (cfg["environment"]["drone_position"]){
          drone_position = cfg["environment"]["drone_position"].as<std::vector<double>>();
      }
  ...
  //
  std::string scene_file = getenv("FLIGHTMARE_PATH") + std::string("/flightpy/configs/scene.yaml");
  // check if configuration file exist
  if (!(file_exists(scene_file))) {
    logger_.error("Unity scene configuration file " + scene_file + " does not exists.");
  }
  // load configuration file
...
//    std::shared_ptr<UnityObject> obj = std::make_shared<UnityObject>(object_id, prefab_id);
    std::shared_ptr<UnityObject> obj = std::make_shared<StaticGate>(object_id);
...
//  TODO: add check if CSV file is empty, cause otherwise it will crash when trying to access elements in the nullptr
    for (auto &row : CSVRange(infile)) {
    // Read column 0 for time
    std::string object_id = "StaticObject" + std::to_string(i);
...
//    std::shared_ptr<UnityObject> obj = std::make_shared<UnityObject>(object_id, prefab_id);
    ...
    i = (int)static_objects_.size();
  }
  num_static_objects_ = (int)static_objects_.size();
  std::cout << num_static_objects_ << "\n";
    return true;
}
...
bool VisionEnv::configCamera(const YAML::Node &cfg) {
  ...
  auto t_BC_vec = cfg["rgb_camera"]["t_BC"].as<std::vector<Scalar>>();
    auto r_BC_vec = cfg["rgb_camera"]["r_BC"].as<std::vector<Scalar>>();
...
//  for (auto & dynamic_object : dynamic_objects_) {
//    bridge->addDynamicObject(dynamic_object);
//  }

//  for (auto & static_object : static_objects_) {
//        bridge->addStaticObject(static_object);
//    }
...
void VisionEnv::disconnectUnity() {
...
int VisionEnv::getNumDetectedObstacles() const { return num_detected_obstacles_; }
