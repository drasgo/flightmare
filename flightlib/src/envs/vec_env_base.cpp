...
template<typename EnvBaseName>
VecEnvBase<EnvBaseName>::~VecEnvBase() = default;
...
bool VecEnvBase<EnvBaseName>::step(Ref<MatrixRowMajor<>> act,
  ...
  if (act.rows() != num_envs_ || act.cols() != act_dim_ ||
      ...
      reward.rows() != num_envs_ || reward.cols() != rew_dim_ ||
          ...
          is_collision.rows() != num_envs_ || is_collision.cols() != 1 ||
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < num_envs_; i++) {
    perAgentStep(i, act, obs, reward, done, is_collision, extra_info);
  }
...
template<typename EnvBaseName>
size_t VecEnvBase<EnvBaseName>::getEpisodeLength() {
  if (envs_.empty()) {
    return 0;
  } else {
...
void VecEnvBase<EnvBaseName>::perAgentStep(int agent_id,
  ...
  Scalar terminal_reward = 0;
    ...
    is_collision[agent_id] = this->envs_[agent_id]->isCollision();
...
template<typename EnvBaseName>
bool VecEnvBase<EnvBaseName>::connectUnity() {
...
void VecEnvBase<EnvBaseName>::isTerminalState(
  const Ref<BoolVector<>>& terminal_state) {}

template<typename EnvBaseName>
void VecEnvBase<EnvBaseName>::sendUnityPing(){
...
template<typename EnvBaseName>
void VecEnvBase<EnvBaseName>::disconnectUnity() {
  if (unity_bridge_ptr_ != nullptr) {
...
template<typename EnvBaseName>
void VecEnvBase<EnvBaseName>::curriculumUpdate() {
